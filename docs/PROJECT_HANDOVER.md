...
2026-09-05 UI 微调：全局顶部栏从 82px 缩至 68px，对局为 58px，3D 对局为 52px；PC 人类回合可在 idle 阶段按 Enter 掷骰，交互控件获得焦点、动画、选棋、AI 回合与按键长按均不会触发。2D 营地动物比例为 0.9、普通航道为 0.68，叠放仍按数量缩放；右侧当前回合标题使用固定 seat 色。左侧 done 状态改用该 seat 的深色并显示白色对勾，3D 状态表现保持原样。

2D 四个独立终点围住的中心格继续仅作装饰，现显示金色奖杯；它不是逻辑落点，不进入 PATH 或 HOMES。
