import {TOTAL, HOME_START_PROG, PATH_LEN, globalIdx, isFlightEntry, isBoostLanding, flightRoute} from './board.js';

// Extracted from the existing game; only state and randomness are injected.
export function rulesFor(G, random = Math.random) {
...
function aiChoose(p, moves) {
  if (p.level === 1) return moves[Math.floor(random() * moves.length)];

  // 聪明电脑硬优先级：精确抵达高于一切其他收益。
  const finishes = moves.filter(mv => mv.kind === 'finish');
  if (finishes.length) return finishes[Math.floor(random() * finishes.length)];
...
return {legalMoves, aiChoose, enemiesAt, previewFlightHitStage};
}
